...
<template>
  <VueDatePicker v-model="reservationDates" class="date-picker w-100" range multi-calendars :enable-time-picker="false"
    :min-date="new Date()" :month-change-on-scroll="false" locale="es" inline auto-apply
    :disabled-dates="disabledDates" />
  <p>Selecciona en el calendario, el rango de días que deseas reservar.</p>
  <v-text-field v-model="formattedDate" label="Check-in --- Check-out" :rules="[v => !!v || 'Falta escoger fechas']"
    required readonly prepend-icon="mdi-calendar" hide-details="auto" variant="solo">
  </v-text-field>
